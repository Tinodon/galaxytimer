...
def load_config():
    ...
    config.setdefault("zone", {})
    return config


def positions(origin, step_x, step_y, limit=UNIVERSE_MAX, zone=None):
    """Parcours ligne par ligne depuis l'origine, dans la zone demandee.
    ...
    """
    # La zone borne le parcours. A deux, chacun prend une bande de Y : sans
    # ca, les deux scans couvrent les memes galaxies et le travail est perdu.
    zone = zone or {}
    x_min = max(origin[0], zone.get("x_min", 0))
    x_max = min(limit, zone.get("x_max", limit))
    y_min = max(origin[1], zone.get("y_min", 0))
    y_max = min(limit, zone.get("y_max", limit))
...
def main():
    ...
    for folder in (chemins.SCAN, chemins.CARTES, chemins.A_TRAITER, chemins.RESULTATS):
    ...
    zone = config["zone"]
    total = sum(1 for _ in positions(origin, step_x, step_y, zone=zone))
    print("Parcours     : depuis {}, pas {} horizontal / {} vertical".format(
        origin, step_x, step_y))
    if zone:
        print("Zone         : X {} a {}, Y {} a {}".format(
            zone.get("x_min", 0), zone.get("x_max", UNIVERSE_MAX),
            zone.get("y_min", 0), zone.get("y_max", UNIVERSE_MAX)))
    ...
    if args.dry_run:
        ...
        for index, position in enumerate(positions(origin, step_x, step_y, zone=zone)):
            if index >= 25:
                break
    ...
    if args.test:
        ...
        plan = positions(origin, step_x, step_y, zone=zone)
